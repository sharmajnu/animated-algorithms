...
var elementSize = { height: 40, width: 80 };
...
function generateArrayContentsForPass(start, end, number) {
    ...
    mainContent.innerHTML += "<div id='numberToFind' class='abs-num' style=" + inlineStyle + ">" + numberToFind + "</div>";
    ...
    for (var i = start; i <= end; i++) {
        ...
        inputArrayHtml += "<div id='" + id + "' class='abs-num' style=" + inlineStyle + "><span class='array-element'>" + inputArray[i] + "</span><span class='array-index'>[" + i + "]</span></div>";
    }
    ...
    $('#infoSectionContainer').css("left", infoLeft + "px");
    $('#infoSectionContainer').css("top", top1 + "px");
    $('#infoSectionContainer').css("display", "block");
...
function doBinarySearch(start, end, number)
{
    ...
    var mid = 0;
    var previousMid = 0;
    addMessageToQueue(start, end, null, "Binary search started for: " + numberToFind)
    while (start <= end)
    {
        ...
        if (numberToFind < parseInt(inputArray[mid]))
        {
            ...
            addMessageToQueue(start, end, mid, numberToFind + " <  " + inputArray[mid] + "(middle element), search in left sub-array.");
            
        }
        else {
            start = mid + 1;
            addMessageToQueue(start, end, mid, numberToFind + " > " + inputArray[mid] + "(middle element), search in right sub-array.");
    ...
    addNotFoundToQueue();
    addMessageToQueue(-1, -1, -1, "Element " + numberToFind + " does not exists in array.");
...
{
    var add = {type:"active", start: start, end:end};
    queue.enqueue(add);
}
function addFoundToQueue(divId, mid)
{
    var add = { type: "found", divId: divId, position: mid };
    queue.enqueue(add);
}

function addNotFoundToQueue() {
    addMessageToQueue(-1, -1, -1, "Element not found.");
    var add = { type: "not-found" };
    queue.enqueue(add);
}

function addMoveToQueue(distance)
{
    var add = { type: "move", direction: distance > 0 ? "right" : "left", distance: Math.abs(distance) }
    queue.enqueue(add);
}

function addComparisonToQueue(divId) {

    var add = { type: "comparison", divId: divId };

    queue.enqueue(add);
}

function addMessageToQueue(start, end, mid, message) {
    var add = { type: "message", start: start, end: end, mid: mid, message: message };
...
function processAnimation() {
    if (!queue.IsEmpty()) {
        ...
        {
            processMessage(element.start, element.end, element.mid, element.message);
        ...
        else if(element.type === "found")
        {
            ...
            $("#infoSection").html(numberToFind + " is found at position '" + element.position +"'");
        }
...
function processMessage(start, end, mid, message)
{
    if (!(start == "-1" && end == "-1"))
    {
        var html = "Active Array[" + start + "....." + end + "], <br/><br/>" + message;
...
function processMoveAnimation(distance, direction)
{
    if(direction === "right")
    {
        $("#numberToFind").animate({ "left": "+=" + distance + "px" }, {
...
function processComparisonAnimation(first, second) {
    ...
    window.setTimeout(function () { removeComparedAndCallProcessAnimation(first, second); }, waitInterval);
}
